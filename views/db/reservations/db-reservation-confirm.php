...
// Inserción solo si viene por POST
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['car-id'])) {
  ...
  $r = mysqli_query($conn, "SELECT car_price_per_day FROM cars WHERE car_id = $car_id");
  if (!($r && mysqli_num_rows($r))) {
    echo "<p class='text-red-600 text-center mt-4'>Car not found.</p>";
    exit;
  }
  $car_price_per_day = (float) mysqli_fetch_assoc($r)['car_price_per_day'];
  ...
  if (is_array($raw_extras)) {
    foreach ($raw_extras as $e) {
      ...
      $qr = mysqli_query($conn, "SELECT extra_id FROM extras WHERE extra_name = '$en' LIMIT 1");
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
  ...
  if ($res && mysqli_num_rows($res)) {
    ...
    $extras_arr = json_decode($row['reservation_extras'], true);

    $pickup = new DateTime($row['pickup_date']);
    $dropoff = new DateTime($row['dropoff_date']);
    $days = max($pickup->diff($dropoff)->days, 1);
    ...
    echo "<p><strong>Reservation #:</strong> {$row['reservation_number']}</p>";
    echo "<p><strong>Customer:</strong> {$row['full_name']} ({$row['user_nif']})</p>";
    echo "<p><strong>Pickup:</strong> {$row['pickup_date']} at {$row['pickup_time']}</p>";
    echo "<p><strong>Dropoff:</strong> {$row['dropoff_date']} at {$row['dropoff_time']}</p>";
